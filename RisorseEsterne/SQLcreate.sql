...
CREATE TABLE IF NOT EXISTS `biblioteca`.`utenti` (
  ...
  `nominativo` VARCHAR(50) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `biblioteca`.`libri` (
  ...
  `Autore` VARCHAR(50) NULL DEFAULT NULL,
  `Titolo` VARCHAR(50) NULL DEFAULT NULL,
  `Edizione` INT(4) NULL DEFAULT NULL,
  `IDU` INT(11) NULL DEFAULT NULL,
  `DataP` DATETIME NULL DEFAULT NULL,
  PRIMARY KEY (`IDL`),
  INDEX `IDU` (`IDU` ASC) VISIBLE,
  CONSTRAINT `libri_ibfk_1`
    FOREIGN KEY (`IDU`)
    REFERENCES `biblioteca`.`utenti` (`IDU`))
ENGINE = InnoDB
AUTO_INCREMENT = 23
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;
